...
TEST_F(FaultFilterRateLimitTest, ResponseRateLimitEnabled) {
  ...
  EXPECT_CALL(encoder_filter_callbacks_, encodeData(BufferStringEqual("hello"), false));
  ...
  EXPECT_CALL(encoder_filter_callbacks_,
              encodeData(BufferStringEqual(std::string(1024, 'a')), false));
  token_timer->callback_();
  ...
  EXPECT_CALL(*token_timer, enableTimer(std::chrono::milliseconds(63)));
  EXPECT_CALL(encoder_filter_callbacks_,
              encodeData(BufferStringEqual(std::string(64, 'a')), false));
  token_timer->callback_();
  ...
  time_system_.sleep(std::chrono::milliseconds(63));
  EXPECT_CALL(encoder_filter_callbacks_,
              encodeData(BufferStringEqual(std::string(64, 'b')), false));
  token_timer->callback_();
  ...
  EXPECT_CALL(encoder_filter_callbacks_,
              encodeData(BufferStringEqual(std::string(1024, 'c')), true));
  token_timer->callback_();
